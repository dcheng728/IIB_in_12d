...
Remarkably, this higher-dimensional effective description is in parallel with what is known between the type IIA theory and M-theory (Table~\ref{tab:iia-iib-uplift}).
The 12d interpretations of the type IIB D7 and D(-1) backgrounds were first identified in \cite{Tseytlin:1996ne}. Here we derive them explicitly, alongside the corresponding type IIA constructions, and use them to read off how the type IIB SL(2, Z) duality acts geometrically in 12d.
...
\bottomrule
\end{tabular}
\caption{The parallel that organises this chapter. Two solutions of eleven-dimensional supergravity reduce on the M-theory circle to the type IIA D6 brane and D0 brane. The same two geometries in twelve dimensions reduce on a torus to the type IIB D7 brane and D($-1$) instanton, the latter after a Wick rotation that renders the instanton Euclidean. No dynamical twelve-dimensional supergravity underlies the right-hand column.}
...
&=
\left(
\partial_\mu+\frac{1}{4}\omega_\mu{}^{ab}\gamma_{ab} -\frac{i}{2}\frac{\partial_\mu \tau_1}{2\tau_2}
\right)\epsilon
...
\end{equation}
Here $Q_\mu = -\frac{\partial_\mu \tau_1}{2\tau_2}$, read off from \eqref{9+1 IIB axio-dilaton Killing spinor equations}, is the non-dynamical U(1) connection built with $\tau$.
...
For the case of a single D7 brane localised at $z_i$ (Figure~\ref{fig:d7-monodromy}), the axio-dilaton profile near $z_i$ is dictated by monodromies to be
\begin{equation}
...
\begin{tikzpicture}[>=Stealth]
  \draw[->] (-0.3,0) -- (5.2,0) node[right] {\footnotesize $\operatorname{Re} z$};
  \draw[->] (0,-0.3) -- (0,3) node[above] {\footnotesize $\operatorname{Im} z$};
  ...
  \node[anchor=north east] at (1.9,1.33) {\footnotesize $z_i$};
\end{tikzpicture}
\caption{The axio-dilaton around a single D7 brane in the transverse complex plane $z$. Encircling the brane at $z_i$ once sends $\tau\to\tau+1$ through the monodromy $\tau\sim\frac{1}{2\pi i}\ln(z-z_i)$, so $\tau$ is single-valued only across a branch cut. Because $\tau_2=-\frac{1}{2\pi}\ln|z-z_i|$ varies logarithmically rather than approaching a constant, the transverse geometry is not asymptotically flat, the hallmark of the co-dimension-two D7.}
...
We will now show that the D7 solution can be interpreted as a 12d KK-monopole geometry compactified on a torus (Figure~\ref{fig:d7-kkmonopole}).
...
\cite{Eguchi:1980jx}
\begin{equation}
ds_d^2 = ds_{1,d-5}^2 + H(\vec{y})d\vec{y}^2 + H(\vec{y})^{-1}(du + \vec{A}\cdot d\vec{y})^2,
...
\begin{figure}[ht]
\centering
% PLACEHOLDER -- replace this tikzpicture with \includegraphics{figures/d7-kk-monopole}
% once the artwork exists. Drawing brief: a horizontal sheet for the R^3 base y; over a
% generic point draw the compact fibre circle u; at the monopole locus the fibre pinches
% to a point (the NUT). Mark the two torus cycles u and v = -y_3. A downward arrow
% "reduce on torus" maps the 12d geometry to the 10d D7, with tau = -A_3 + i H read off
% from the one-form du + tau dv.
\begin{tikzpicture}
  \node[draw, dashed, rounded corners, fill=gray!3,
       ...
       {Placeholder for hand-drawn artwork.\\ See the caption and the source comment for the drawing brief.};
\end{tikzpicture}
\caption{The twelve-dimensional Kaluza-Klein monopole that reduces to the D7 brane. The compact circle $u$ is fibred over the three-dimensional base $\vec y$ and pinches off at the monopole locus (the NUT). Compactifying the further direction $v=-y_3$ makes the fibre a torus, and reducing on that torus gives the ten-dimensional D7 brane, with axio-dilaton $\tau=-A_3+iH$ read off from the fibration one-form $du+\tau dv$.}
\label{fig:d7-kkmonopole}
\end{figure}
...
We will recognise this $S_1$ as the M-theory circle.
...
u\to -y_3 .
\end{equation}
We can thus interpret the type IIB SL(2, Z) duality transformations as large gauge transformations in 12d, so that the discrete S and T dualities, ordinarily postulated in 10d, arise geometrically in this 12d picture.
...
In $d$ dimensions, written in light-cone coordinates $v,u = y\pm t$, they take the form
\begin{equation}
...
By specialising \eqref{d-dim pp-wave} to 11d, we find the 11d pp-wave solution
\begin{equation}
ds_{11}^2
=
...
We now discuss a further consistency check for the relation between 12d gravity and the type IIB axio-dilaton sector, namely how the 1/2 BPS condition of the latter can be obtained by reducing the covariantly-constant equation of the former.
...
\end{equation}
Under a local supersymmetry transformation parameterised by a spinor $\epsilon$, one can write down a general ansatz for the Killing spinor equation of the gravitino from covariance alone
\begin{equation}
\delta\psi_M
= \left\{
\nabla_M + \sum_n \left[
...
\end{equation}
with summation over the form fields of the given theory, and $c_n, d_n$ are constants to be determined.
...
\end{aligned}
\end{equation}
For the standard Kaluza-Klein ansatz \eqref{string frame KK reduction ansatz}, and substituting the reduced spin connection (see \cite{Becker:2006dvp}), we find
\begin{equation}
\begin{aligned}
...
+\frac{1}{2}\omega_m{}^{12,n}\Gamma_{12,n}
+\frac{1}{2}\omega_m{}^{11,12}\Gamma_{11,12}\right)\epsilon = 0,
...
\begin{equation}
\omega_m{}^{11,n}=\omega_m{}^{12,n}=0,\quad
\omega_m{}^{11,12} = -\frac{1}{2}\frac{\partial_m \tau_1}{\tau_2}.
\end{equation}
The 12d covariant derivative \eqref{12d covariant derivative} thus becomes
\begin{equation}
\left[
\nabla_m -\frac{i}{4}\frac{\partial_m(\tau+\bar\tau)}{\tau-\bar\tau}\Gamma_{11,12}
\right]\epsilon.
\end{equation}
