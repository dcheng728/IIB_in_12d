...
Remarkably, this higher-dimensional effective description is in parallel with what is known between the type IIA theory and M-theory.
...
\label{D7 solution}
\end{equation}
where $\tau = C^{(0)} + i e^{-\Phi}$ is the axio-dilaton, and takes the form
\begin{equation}
...
For the case of a single D7 brane localised at $z_i$, the axio-dilaton profile near $z_i$ is dictated by monodromies to be
\begin{equation}
...
In $d$ dimensions, written in light-cone coordinates $u,v = y\pm t$, they take the form
\begin{equation}
...
\begin{equation}
\begin{aligned}
\widehat{\nabla}_m \epsilon = \nabla_m \epsilon + \frac{1}{4} \hat{w}_{mBC} \Gamma^{BC} \epsilon &= 0,\\
...
\begin{equation}
\begin{aligned}
\tfrac14 e^{\phi/3}\left[-\tfrac12 e^\phi F_{bc}\Gamma^{bc}-\tfrac43\partial_m\phi\Gamma^m\Gamma^{11}\right]\epsilon &= 0\\
e^{\phi/3}e^m{}_a\left[\nabla_m-\tfrac16\Gamma_m{}^n\partial_n\phi+\tfrac14 e^\phi F_{mn}\Gamma^n\Gamma^{11}\right]\epsilon &= 0,
\label{IIA spacetime gravitino}
\end{aligned}
\end{equation}
The first equation is algebraic, and using $(\Gamma^{11})^2=1$ it becomes
\begin{equation}
...
\end{equation}
Finally, the Weyl rescaling $\epsilon\to e^{-\phi/6}\epsilon$ removes the $\tfrac16\partial_m\phi$ term, leaving the type IIA Killing spinor equation in the gravity, KK-vector and dilaton sector,
\begin{equation}
\delta \psi_m =
\left(
...
\end{equation}
with $F_{mn}=\partial_m A_n-\partial_n A_m$. This is eq.~(8.51) of \cite{Becker:2006dvp} restricted to vanishing $H_3$ and $F_4$.
...
+\frac{1}{2}\omega_m{}^{10,11}\Gamma_{10,11}\right)\epsilon = 0,
...
Here we have used that, on the Euclidean torus,
\begin{equation}
\Gamma_{10,11} = \Gamma_{10}\Gamma_{11},\quad
(\Gamma_{10,11})^2 =-\Gamma_{10}^2\Gamma_{11}^2 = -1,
\end{equation}
so that $\Gamma_{10,11}$ is, up to a similarity transformation, $i$ times the U(1) generator.
This would also hold had we compactified a non-compact torus instead, to obtain a Euclidean type IIB theory: one then compactifies on a 1+1 torus, and a factor of $i$ arises both in identifying the U(1) generator from $\Gamma_{11,12}$ and in defining the Euclidean compact scalar $C = i\mathcal C$, so that the 12d covariant derivative again reproduces the type IIB gravitino variation.
