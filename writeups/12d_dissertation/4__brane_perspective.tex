...
The D7 brane found above is the simplest of the type IIB $(p,q)$ 7-branes, a family permuted by \SLZ{}. 
...
We have already identified the type IIB \SLZ{} with the large diffeomorphisms in 12d, through the generators $T$ and $S$ above, so it remains to see how \SLZ{} is built into the torus itself. 
We thus briefly review how \SLZ{} arises from periodically identifying coordinates on a torus.


A torus has topology $S^1 \times S^1$. 
We adopt coordinates $\vec{\sigma} \in \mathbb{R}^2$ by periodically identifying them as $(\sigma^1,\sigma^2)\sim (\sigma^1,\sigma^2) + 2\pi (m,n)$ for integers $m,n$.
Using (Diffeomorphism $\times$ Weyl) symmetry, it is always possible to bring the metric to the flat form $ds^2 = (d\sigma^{1})^2+(d\sigma^{2})^2$ \cite{Polchinski:1998rq}, but depending on the torus, the periodicity condition will become that of a parallelogram
\begin{equation}
(\sigma^1,\sigma^2) \sim (\sigma^1,\sigma^2) + 2\pi (m+nv^1,n v^2),
\end{equation}
for some $\vec{v} \in \mathbb{R}^2$ that defines the parallelogram.
The equivalence class is that of a 2d lattice.

We can equivalently describe coordinates on the torus with $\mathbb{C}$ instead of $\mathbb{R}^2$.
It turns out that instead of using the orthonormal basis $1, i$ in $\mathbb{C}$, it is more convenient to use the basis $1, \tau$, where $\tau \equiv v^1 + i v^2$ is a complex number with $\text{Im} \tau \neq 0$.
The periodicity condition then becomes
\begin{equation}
\sigma^1 + \tau \sigma^2 \sim \sigma^1 + \tau \sigma^2 + (m + n\tau),
\end{equation}
and the metric is
\begin{equation}
ds^2 = |d\sigma^1 + \tau d\sigma^2|^2.
\end{equation}
Here $\sigma^1, \sigma^2$ are the coordinates in the basis $1, \tau$, scaled to range in $[0,1]$.

As a consequence of the periodic identification on the coordinates, the torus modulus $\tau$ is only identified up to equivalence relations that preserve the lattice.
Certain transformations $\tau \to \tau'$ may be realised by lattice automorphisms that leave the torus geometry invariant.
There are two types of such transformations:
\begin{enumerate}
    \item $\tau \to \tau+1$ is equivalent to $\tau \to \tau$, $(m, n) \to (m+n,n)$
    \item $\tau \to -1/\tau$ is equivalent to $\tau \to \tau$, $(m, n) \to (n,-m)$, followed by a constant rescaling of the coordinates that restores the metric to standard form.
\end{enumerate}

These two operations generate the \SLZ{} transformations on $\tau$: $\tau \to \frac{a\tau +b}{c\tau + d}$, $ad-bc = 1$ (Figure~\ref{fig:st-cycles}). 
The \SLZ{} symmetry of the torus therefore reflects the periodic identifications imposed in placing coordinates on $S^1 \times S^1$: these fix the lattice but not a basis for it, and \SLZ{} relates the equivalent choices of basis. The integers $(m,n)$ label the 1-cycles of $T^2$, on which \SLZ{} acts by relabelling.
...
\right]\epsilon,
\end{equation}
where $Q_m = -\frac{1}{2} \frac{\partial_m \tau_1}{\tau_2}$ is the non-dynamical U(1) connection of the type IIB axio-dilaton sector.
