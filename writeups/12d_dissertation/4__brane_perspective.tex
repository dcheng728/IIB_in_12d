...
The D7 brane found above is the simplest of the type IIB $(p,q)$ 7-branes, a family permuted by \SLZ{} \cite{Weigand:2018rez}. 
We now give this family a geometric origin in 12d. The claim is that a $(p,q)$ 7-brane is the 12d KK-monopole on $T^2$ whose Taub-NUT circle is wrapped on the $(p,q)$ cycle of the torus, and that the \SLZ{} acting on the brane charges $(p,q)$ is the same \SLZ{} that acts on the $(p,q)$ cycles of $T^2$. 

Having identified the type IIB \SLZ{} with the large 12d diffeomorphisms $T$ and $S$ above, we now recall how the $T$ and $S$ generators of \SLZ{} arise from the periodic identifications that define a torus. 
...
Otherwise, writing $d \equiv \gcd(p,q)$, it is $d$ disjoint copies of the primitive $(p/d,\,q/d)$ loop.
\SLZ{} permutes these primitive 1-cycles among themselves, taking any one to any other.

We can now draw the two threads together. 
We have seen that the type IIB D7 brane is obtained by reducing the 12d KK-monopole on $T^2$, where the Taub-NUT circle is identified with one of the $S^1$ factors of $T^2$, which we can take to be the $(1,0)$.
Furthermore, we identified the type IIB $S, T$ generators of \SLZ{} with large diffeomorphisms of the reduction torus.
...
\begin{tikzpicture}[>=Stealth, scale=1.7, every node/.style={font=\footnotesize}]
  ...
  \end{scope}
\end{tikzpicture}
\caption{The generators $T$ and $S$ acting on the lattice basis $\{1,\tau\}$, the two periods of the torus. $T:\tau\to\tau+1$ slides the oblique generator over by one period, a shear, while $S:\tau\to-1/\tau$ swaps the two generators, up to a rescaling that restores unit length. On the 1-cycles $m\cdot1 + n\cdot\tau$ these act as $(m,n)\to(m+n,n)$ and $(m,n)\to(n,-m)$, generating every coprime $(p,q)$ class from the $(1,0)$ cycle of the D7 brane.}
\label{fig:st-cycles}
\end{figure}
