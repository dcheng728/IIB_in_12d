...
Having identified the type IIB \SLZ{} with the large 12d diffeomorphisms $T$ and $S$ above, we now recall how the same \SLZ{} arises from the periodic identifications that define a torus. 
...
These two operations generate the \SLZ{} group: $\tau \to \frac{a\tau +b}{c\tau + d}$, $ad-bc = 1$ (Figure~\ref{fig:st-cycles}). 
The \SLZ{} symmetry of the torus therefore reflects the periodic identifications imposed on $S^1 \times S^1$: these fix the lattice but not a basis for it, and \SLZ{} relates the equivalent choices of basis. 
The 1-cycles of $T^2$ are labelled by integer pairs $(p,q)$: the class $p + q\tau$ is a loop winding $p$ and $q$ times around the two circles, represented by a single connected loop when $p$ and $q$ are coprime. 
\SLZ{} permutes these coprime cycles among themselves, relating any one to any other.

We can now draw the two threads together. The type IIB \SLZ{} is realised in 12d by the generators $T$ and $S$ as large diffeomorphisms of the reduction torus \eqref{12d KK-monopole in complex coordinates}. By the review above these are precisely the lattice relabellings $(m,n)$, the action of \SLZ{} on the 1-cycles of $T^2$. The Taub-NUT circle of the KK-monopole is one such 1-cycle, the $(1,0)$ cycle for the D7 brane \eqref{D7 solution}, and acting with the duality carries it onto a general $(p,q)$ cycle (Figure~\ref{fig:pq-cycle}). A $(p,q)$ 7-brane is therefore the 12d Kaluza-Klein monopole whose Taub-NUT circle is wrapped on the $(p,q)$ cycle of $T^2$. In particular, the \SLZ{} that permutes the 7-brane charges $(p,q)$ is the same \SLZ{} that acts on the $(p,q)$ cycles of the torus.
