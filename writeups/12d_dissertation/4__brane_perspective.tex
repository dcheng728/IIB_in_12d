...
z_1\to -w_3 .
\end{equation}
We can thus identify the type IIB \SLZ{} duality transformations as large gauge transformations in 12d.
To understand what geometric interpretation such an identification buys us, we briefly review the relation between \SLZ{} and the modulus of a torus.

\paragraph{Geometric Interpretation of the (p, q) 7-brane and \SLZ{}}

We now build up the geometric interpretation of the discussion above, that the (p, q) 7-brane can be understood as a 12d KK-monopole on $T^2$.
We begin by recalling the geometry of a torus, and how its modulus $\tau$ is related to the periodicity conditions one has to impose when putting coordinates on $S^1 \times S^1$.
...
Distinct values of $\tau$, however, need not correspond to distinct tori. Identifying the equivalence relations on $\tau$ is therefore necessary in a number of settings, such as determining the moduli space of the type IIB theory, or the cycle wrapped by a brane.
These are the transformations of $\tau$ that preserve the lattice, of which there are two generators:
\begin{enumerate}
...
\begin{tikzpicture}[>=Stealth, scale=0.8, every node/.style={font=\footnotesize}]
  ...
  \begin{scope}
    \begin{scope}
      \clip (-1.4,-0.6) rectangle (2.9,2.7);
      \fill[red!10] (0,0) -- (1,0) -- (2.5,1.2) -- (1.5,1.2) -- cycle;
      \foreach \m in {-3,-2,-1,0,1,2,3,4} \foreach \n in {-1,0,1,2,3}
        \fill[black!55] (\m+0.5*\n,1.2*\n) circle (1.3pt);
      \draw[densely dashed,black!45] (0,0) -- (1,0) -- (1.5,1.2) -- (0.5,1.2) -- cycle;
    \end{scope}
    \draw[->,gray] (0,0) -- (0.5,1.2);
    \draw[->,red!70!black] (0.5,1.2) to[bend left=22] (1.5,1.2);
    \draw[->,thick] (0,0) -- (1,0) node[below right] {$1$};
    \draw[->,thick] (0,0) -- (1.5,1.2) node[above right] {$\tau+1$};
    \node[gray] at (0.30,1.32) {$\tau$};
    \node at (0.75,-1.15) {$T:\ \tau\to\tau+1$};
  \end{scope}
  % panel 2: S : tau -> -1/tau
  \begin{scope}[shift={(5.6,0)}]
    \begin{scope}
      \clip (-1.4,-0.6) rectangle (2.9,2.7);
      \fill[red!10] (0,0) -- (0.5,1.2) -- (-0.5,1.2) -- (-1,0) -- cycle;
      \foreach \m in {-3,-2,-1,0,1,2,3,4} \foreach \n in {-1,0,1,2,3}
        \fill[black!55] (\m+0.5*\n,1.2*\n) circle (1.3pt);
      \draw[densely dashed,black!45] (0,0) -- (1,0) -- (1.5,1.2) -- (0.5,1.2) -- cycle;
    \end{scope}
    \draw[->,gray] (0,0) -- (1,0) node[black!55,below right] {$1$};
    \draw[->,thick] (0,0) -- (0.5,1.2) node[above right] {$\tau$};
    \draw[->,thick] (0,0) -- (-1,0) node[below left] {$-1$};
    \node at (0.75,-1.15) {$S:\ \tau\to-1/\tau$};
  \end{scope}
\end{tikzpicture}
\caption{The action of the generators $T$ and $S$ on the lattice, the dashed parallelogram marking the original cell $\{1,\tau\}$ in each panel. The transformation $T:\tau\to\tau+1$ shears the basis, $(m,n)\to(m+n,n)$, replacing the cell by $\{1,\tau+1\}$, while $S:\tau\to-1/\tau$ swaps the two generators with a sign, $(m,n)\to(n,-m)$, replacing it by $\{\tau,-1\}$. In both cases the lattice, and hence the torus, is left unchanged. Applied to the Taub-NUT circle, this is the relabelling that takes the D7 brane to a general $(p,q)$ 7-brane.}
\label{fig:st-lattice}
\end{figure}

Since the Taub-NUT circle of the 12d KK-monopole is a definite 1-cycle of the reduction torus, an SL(2,Z) transformation changes which cycle it wraps.
