...
\paragraph{Geometric Interpretation of the $(p,q)$ 7-brane and \SLZ{}}

The D7 brane found above is the simplest of the type IIB $(p,q)$ 7-branes, a family permuted by the \SLZ{} duality. We now give this family a geometric origin in 12d. The claim is that a $(p,q)$ 7-brane is the 12d KK-monopole on $T^2$ whose Taub-NUT circle is wrapped on the $(p,q)$ cycle of the torus, and that the \SLZ{} acting on the brane charges $(p,q)$ is the same \SLZ{} that acts on the $(p,q)$ cycles of $T^2$. 
...
A torus has topology $S^1 \times S^1$, suppose its coordinates are given by $(\sigma^1,\sigma^2)\sim (\sigma^1,\sigma^2) + 2\pi (m,n)$.
Its line element is $ds^2 = \gamma_{ab}d\sigma^a d\sigma^b$ for some $\gamma_{ab}$. 
Using (Diffeomorphism $\times$ Weyl) symmetry, it is always possible to bring the metric to the flat form $ds^2 = (d\sigma^{1})^2+(d\sigma^{2})^2$ \cite{Polchinski:1998rq}, 
but depending on the torus, the periodicity condition is no longer that on a rectangle, but a rotated parallelogram
\begin{equation}
(\sigma^1,\sigma^2) \sim (\sigma^1,\sigma^2) + 2\pi (m+nv^1,n v^2).
\end{equation}
So far we have used the orthonormal basis $\vec{e}_1, \vec{e}_2$ of $\mathbb R^2$, but the lattice may equally be described in $\mathbb C$ with basis vectors $1$ and $\tau$, linearly independent provided $\text{Im} \tau \neq 0$.
For clearer notation, we also scale the coordinates $\vec{\sigma}$ so that they range in [0,1] instead of [0,2$\pi$].
The periodicity condition and metric become
\begin{equation}
\sigma^1 + \tau \sigma^2 \sim \sigma^1 + \tau \sigma^2 + (m + n\tau),\quad
ds^2 = |d\sigma^1 + \tau d\sigma^2|^2.
\end{equation}
This is the standard form of the torus metric, with $\tau$ its modulus.
Distinct values of $\tau$, however, need not correspond to distinct tori. 
We thus need to identify the equivalence relations on $\tau$ that preserve the lattice, and thus the torus geometry.
\begin{enumerate}
    \item $\tau \to \tau + 1$ is equivalent to $\tau \to \tau$, $(m,n) \to (m+n,n)$.
    \item $\tau \to -1/\tau$ is equivalent to $\tau \to \tau$, $(m,n) \to (n,-m)$, followed by a constant rescaling $\vec{\sigma} \to \tau \vec{\sigma}$ that restores the metric to standard form. 
\end{enumerate}

These two operations generate the \SLZ{} transformations on $\tau$: $\tau \to \frac{a\tau +b}{c\tau + d}$, $ad-bc = 1$ (Figure~\ref{fig:st-cycles}). So this is how one should think of the \SLZ{} symmetry of the torus: it is a footprint of the periodic identification one has to make when putting coordinates on $S^1 \times S^1$. In particular the integers $(m,n)$ label the 1-cycles of $T^2$, and \SLZ{} acts by relabelling them.
...
\begin{tikzpicture}[>=Stealth, scale=1.7, every node/.style={font=\footnotesize}]
  ...
  \end{scope}
\end{tikzpicture}
\caption{The generators $T$ and $S$ acting on the lattice basis $\{1,\tau\}$, the two periods of the torus. $T:\tau\to\tau+1$ slides the oblique generator over by one period, a shear, while $S:\tau\to-1/\tau$ swaps the two generators, up to a rescaling that restores unit length. On the 1-cycles $m\cdot1 + n\cdot\tau$ these act as $(m,n)\to(m+n,n)$ and $(m,n)\to(n,-m)$, generating every $(p,q)$ class from the $(1,0)$ cycle of the D7 brane.}
\label{fig:st-cycles}
\end{figure}
