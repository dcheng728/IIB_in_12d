...
Reducing the 11d covariantly-constant condition $\widehat\nabla_M\epsilon=0$ on the standard Kaluza--Klein ansatz
\begin{equation}
ds^2_{11}=e^{-2\phi/3}g_{mn}dx^m dx^n+e^{4\phi/3}(dx^{11}+A_m dx^m)^2,
\end{equation}
in which the 11d metric repackages the 10d metric $g_{mn}$, the dilaton $\phi$, and the KK-vector $A_m$ (with the eleventh coordinate $x^{11}$ an isometry and $\Gamma^{11}$ the gamma matrix along the circle, i.e. the 10d chirality operator), the condition $\widehat\nabla_M\epsilon=0$ splits into one component along the circle and ten along the spacetime directions.
The internal component is algebraic and reduces to the type IIA dilatino variation, while the spacetime components, after the standard Weyl rescaling $\epsilon\to e^{-\phi/6}\epsilon$ and the redefinition of the gravitino that mixes in the dilatino, give the type IIA Killing spinor equation in the gravity, KK-vector and dilaton sector \cite{Becker:2006dvp}:
\begin{equation}
\delta \psi_m =
\left(
\nabla_m - \frac{1}{8}e^\phi F_{np}\Gamma_m{}^{np}\Gamma^{11}
\right)\epsilon = 0,
\label{IIA Killing spinor equation}
\end{equation}
with $F_{mn}=\partial_m A_n-\partial_n A_m$, which is eq.~(8.51) of \cite{Becker:2006dvp} restricted to vanishing $H_3$ and $F_4$.
...
This would also hold had we compactified a non-compact torus instead, to obtain a Euclidean type IIB theory\footnote{
