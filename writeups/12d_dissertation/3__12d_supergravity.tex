...
The BPS states in the $\mathrm{OSp}(1|32)$ algebra have been studied \cite{Ueno:1999xa}, and the various consistent fractions of preserved supersymmetry were worked out \cite{Manvelyan:1998kx}.
At an algebraic level, ideas for the unification of the various dualities in 12 and 13 dimensions \cite{Bars:1996dz,Bars:1996us,Bars:1996ab,Bars:1996hr}, and even 14 dimensions, have been suggested \cite{Rudychev:1997ui,Bars:1997ug}.
...
The little group of an SO(10, 2) theory is the non-compact SO(9, 1), which has no nontrivial finite-dimensional unitary representations \cite{Weinberg:1995mt}.
...
\end{equation}
Here $\Gamma_n$ denotes the antisymmetrised gamma-matrix structures and $Z_n$ the corresponding central charges (the 2-form and self-dual 6-form charges of \eqref{OSp algebra}). Because the RHS is symmetric and positive definite, we can diagonalise it.
...
On-shell states are counted against the compact little group SO(10), that of a one-time $11+1$ dimensional theory.
But the gravitino representation of SO(10) already has 144 states, exceeding the budget for fermions. 
There is indeed a way to reach 144 + 144 states for SO(10) without too many scalars: a single gravitino ($144$) in the fermion sector, plus a graviton ($54$) and two 2-forms ($2\times 45 = 90$) in the bosonic sector. 
This matches no known 10d or 11d multiplet.
...
\right]
+\frac{\sqrt{3}}{4}\int \hat{C}_4\wedge \hat{F}_4 \wedge \hat{F}_4
\label{Khviengia action}
\end{equation}
where $\hat{F}_5 = d\hat{C}_4$, $\hat{F}_4 = d\hat{C}_3$, $[\kappa_{12}^2]=L^{10}$, and $\hat{\Phi}$ here is a 12d dilaton-like scalar.
We will use hats $\hat{\phantom{x}}$ for higher-dimensional fields and straight letters $g_{mn},C_n,F_{n+1}$ for lower-dimensional fields. 
...
\label{Choi action}
\end{equation}
Here $\hat{F}_5 = d\hat{C}_4$ and $\hat{F}_4 = d\hat{C}_3$. 
Upon closer examination, we find \eqref{Choi action} is just 11d supergravity integrated over a spectator dimension.
To see this, we split the 12d space as $11+1$, an 11d base $x^m$ together with a single extra coordinate $y$, in contrast to the $10+2$ torus split $(x^m,z_1,z_2)$ used in Section~\ref{sec:covariant-unification}. Following \cite{Choi:2014vya,Choi:2015gia}, we package all dependence on the twelfth coordinate $y$ into a single scalar $r\equiv r(x,y)$, which also enters the metric:
\begin{equation}
\hat{C}_4(x,y) \equiv r(x,y) C_3\wedge dy,\quad
\hat{F}_5(x,y) \equiv r(x,y) F_4\wedge dy,
\end{equation}
\begin{equation}
\hat{g}_{mn} = g_{mn},\quad
\hat{g}_{my} = 0,\quad
\hat{g}_{yy} = r^2,
\end{equation}
where $F_4 = dC_3$ and straight letters denote 11d fields.
Substituting these into \eqref{Choi action} and evaluating $\int d^{11}x dy \sqrt{-\hat{g}} \hat{R}$, the action reduces to
\begin{equation}
2\kappa_{12}^2 S =
...
\end{equation}
This is nothing but the bosonic action of 11d supergravity, multiplied by an auxiliary field $\lambda(x)$ whose equation of motion imposes that the 11d bosonic Lagrangian vanishes.
In an attempt to unify the various string theory dualities, an SL(2, R)$\times \mathbb{R}^+$ Exceptional Field Theory was proposed \cite{Berman:2015rcc,Rudolph:2017vzy}.
