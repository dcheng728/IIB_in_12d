...
Packaging all $y$-dependence into a single scalar $r(x^m,y)$ via $\hat{C}_4 = r(x^m,y) C_3(x)\wedge dy$, $\hat{F}_5 = r(x^m,y) F_4\wedge dy$ with $F_4 = dC_3$, and $\hat{g}_{mn}=g_{mn}(x)$, $\hat{g}_{my}=0$, $\hat{g}_{yy}=r(x^m,y)^2$, the action \eqref{Choi action} reduces to $S = \int d^{11}x\left[\sqrt{-g}\left(R-\tfrac12|F_4|^2\right)+\tfrac16\int C_3\wedge F_4\wedge F_4\right]\lambda(x)+\text{boundary}$, with $\lambda(x)\equiv\int dy r(x^m,y)$. This is nothing but the bosonic action of 11d supergravity multiplied by an auxiliary field $\lambda(x)$, whose equation of motion merely imposes that the 11d bosonic action vanish; the twelfth dimension carries no dynamics.
}.
