...
In general, they are complex, but for the Lorentz group SO(10, 2)\footnote{Here $s$ and $t$ are integer entries in the special orthogonal group SO(s, t).}, because
\begin{equation}
s-t=0\mod 8,
\end{equation}
...
The BPS states in the OSp(1$|$32) algebra have been studied \cite{Ueno:1999xa}, and the various consistent fractions of preserved supersymmetry were worked out \cite{Manvelyan:1998kx}.
...
In any known one-time supergravity theory with 32 supercharges there are 128+128 states, suggesting the same starting point.
But the gravitino representation of SO(10) already has 144 states, exceeding the budget for fermions. There is indeed a way to add up to 144 + 144 states for SO(10) without too many scalars: a single gravitino in the fermion sector plus a graviton and two 2-forms in the bosonic sector. This is not related to any known 10d or 11d multiplets.
...
There have been some attempts at 10+2d SYM \cite{Nishino:1996wp,Sezgin:1997gr,Nishino:1997hk,Nishino:1997ia,Nishino:1999ck} and 10+2d supergravity \cite{Nishino:1997gq,Nishino:1997sw,Nishino:1998qn}, but none of them has achieved satisfactory results, as they either introduce null projectors that explicitly break SO(10, 2),
...
After it was shown that the (2+2)-brane can propagate in 10+2d spacetime \cite{Blencowe:1988sk}, the $\mathcal N=(2,1)$ string was studied \cite{Kutasov:1996fp, Kutasov:1996zm, Martinec:1996wn}. The $\mathcal N=1$ sector contains an effective 10+2d target space, with the $\mathcal N=2$ sector on a 2+2d target.
Later, a $\mathcal{N}=1$ superstring with a 2+2d target space was constructed \cite{Khviengia:1995pm,Lu:1995pn}.
...
In parallel, a Green-Schwarz type super 2+2d brane embedded in 10+2d background framework was constructed \cite{Hewson:1996yh,Hewson:1997wv,Hewson:1998sw}.

There were also investigations on higher dimensional bosonic field theories, that upon compactifications and truncations, may produce the known theories.
...
-\frac{1}{2} e^{\frac{2i}{\sqrt{5}} \hat{\Phi}}|\hat{F}_5|^2
...
We will use hats $\hat{\phantom{x}}$ for higher dimensional fields and standard letters $g_{mn},C_n,F_{n+1}$ for lower dimensional fields. 
...
To see this, we follow \cite{Choi:2014vya,Choi:2015gia} and package all dependence on the twelfth coordinate $y$ into a single scalar $r\equiv r(x^m,y)$, which also enters the metric:
\begin{equation}
\hat{C}_4(x^m,y) \equiv r(x^m,y) C_3(x)\wedge dy,\quad
\hat{F}_5(x^m,y) \equiv r(x^m,y) F_4\wedge dy,
\end{equation}
\begin{equation}
\hat{g}_{mn}(x) = g_{mn}(x),\quad
\hat{g}_{my} = 0,\quad
\hat{g}_{yy}(x,y) = r(x,y)^2,
\end{equation}
where $F_4 = dC_3$ and straight letters denote 11d fields.
Substituting these into \eqref{Choi action} and evaluating $\int \sqrt{-\hat{g}} \hat{R}$, the action reduces to
\begin{equation}
S =
\int d^{11}x
\left[
...
\end{equation}
This is nothing but the bosonic action of 11d supergravity, multiplied by an auxiliary field $\lambda(x)$ whose equation of motion imposes that the 11d bosonic action vanishes.
In an attempt to unify the various string theory dualities, an SL(2, R)$\times R^+$ Exceptional Field Theory was proposed \cite{Berman:2015rcc,Rudolph:2017vzy}.
...
The persistent obstructions reviewed above (the non-compact little group, the absence of a momentum generator in the OSp(1$|$32) algebra, and the failure to match known 10d or 11d multiplets) suggest that a dynamical 10+2d supergravity with 32 supercharges reducing to the type IIB theory is not available.
