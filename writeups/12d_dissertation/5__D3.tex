...
\begin{aligned}
S_{D3}
&=\int d^4x \sqrt{-\det\big(\hat{g}_{mn} + e^{-\hat \Phi/2} (\mathcal{F}_2)_{mn}\big)}\\
&\quad+\int \Big(\hat C_4 + \hat C_2\wedge \mathcal{F}_2 + \tfrac{1}{2} C \mathcal{F}_2\wedge\mathcal{F}_2\Big)
+\text{higher order},\qquad
\mathcal{F}_2 \equiv dA+\hat B_2 .
\end{aligned}
\label{D3 action}
\end{equation}
Here hats denote bulk fields pulled back onto the brane worldvolume, $m,n$ label worldvolume indices, $(\mathcal{F}_2)_{mn}$ are the components of the worldvolume 2-form $\mathcal{F}_2$, and $C$ is the RR axion.
...
To dualise, we promote $\mathcal F_2$ to an independent field and enforce its Bianchi identity with a dual potential $\tilde A$ as Lagrange multiplier, whose field strength $\tilde F_2 = d\tilde A$ enters through the term $\int \tilde F_2\wedge\mathcal F_2$. Varying the now-independent $\mathcal F_2$ gives the algebraic relation
\begin{equation}
