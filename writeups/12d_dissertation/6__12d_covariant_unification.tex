...
\right],
\end{equation}
the equations of motion, together with the Bianchi identity (assuming $F = dA$), are given by
\begin{equation}
\begin{aligned}
R_{MN}
...
In this section, we gather the various 12d insights obtained in the previous section from the type IIB branes, and propose a covariant 12d unification of the $SL(2, R)\times SO(9,1)$ form fields.
We will use hats $\hat{\phantom{x}}$ to denote fields in the higher dimension.
The 10d metric $g_{mn}$ with $m,n=0,1,...,9$ will be interpreted as embedded inside a 12d metric $\hat{g}_{MN}$, with $M,N=0,1,...,11$. We will use $\hat R$ to denote the Ricci scalar computed with $\hat{g}_{MN}$, and use $\hat{F}_{n+1} = d\hat{C}_{n}$ to denote form fields in 12d.

Let the 12d coordinates be parameterised by $(x^m,z_1,z_2)$, and let $\hat{g}_{ab}$ be the $2\times 2$ metric on the torus.\footnote{
...
\right)
&=
\frac{\mathrm{Vol}(T_2)}{2\kappa_{12}^2}\int d^{10}x \sqrt{-\hat{g}} \hat{R}\\
...
&=\det(\hat{g}^{ab})|F_5|^2\bigg|_{g_{mn}} = |F_5|^2\bigg|_{g_{mn}}.
\end{aligned}
\end{equation}
Then we may define a 12d 7-form
\begin{equation}
\widetilde{\hat{F}}_7 \equiv \hat{F}_7 - \frac{1}{2}\hat{C}_3 \wedge \hat{F}_4
\end{equation}
that exactly supplies the type IIB composite 5-form contribution upon contraction in 12d:
\begin{equation}
|\widetilde{\hat{F}}_7|^2\bigg|_{\hat{g}_{MN}} =
\det(\hat{g}^{ab})|\tilde F_5|^2\bigg|_{g_{mn}}.
\end{equation}
Writing $\widetilde{\hat{F}}_5\equiv\tilde F_5$ for the corresponding torus-leg-free five-form uplift, the reduction in Appendix~\ref{sec:form-field-reduction} gives $\widetilde{\hat{F}}_7=\widetilde{\hat{F}}_5\wedge dz_1\wedge dz_2$.
...
\end{equation}
We note that reproducing the type IIB Chern-Simons term in 12d necessitates both the 4-form field strength $\hat F_4$ and the 4-form potential $\hat C_4$, whose field strength is the 5-form $\hat F_5$.
...
\sqrt{-\hat{g}}
\left(
\hat{R} - \frac{1}{2}|\hat F_4|^2-\frac{1}{4}|\widetilde{\hat{F}}_7|^2
\right)-\frac{1}{8\kappa_{12}^2}\int_{T_2\times \mathbb{R}^{1,9}}\hat{C}_4\wedge \hat{F}_4\wedge \hat{F}_4.
\end{aligned}
\label{12d action}
\end{equation}
The $\hat F_7$ does not arise from an independent degree of freedom, it is related to $\hat C_4$ by $*_{12}d\hat C_4 = -\hat F_7$.
The action \eqref{12d action} is exactly the Einstein frame type IIB action \eqref{eq:sl2r_invariant_einstein_frame_action} with $g_s=1$.
...
Although one often thinks of F1 and NS5, D1 and D5 as EM dual pairs, their $H_3$ and $F_3$ configurations are not related by EM duality in 10d, and of course they have opposite dilaton profiles as well\footnote{The brane solutions are listed in \ref{sec:10d IIB branes}}.
...
+H^{-1/2}dz_1^2
+H^{1/2}dz_2^2\\
(\hat{F}_4)_{m_1 m_2 m_3 z_2}
&= \varepsilon_{m_1 m_2 m_3 m}\partial_m H
\end{aligned}
\end{equation}

% \subsection{Are the uplifts genuine 12d solutions?}
% \label{sec:genuine-12d-solutions}
% The uplifts of the previous subsection solve the 10d type IIB equations of motion by construction, since substituting the reduction ansatz of Section~\ref{sec:a-covariant-unification-in-12d} into the repackaged action \eqref{12d action} returns the 10d action.
% It is a separate question whether they also solve the equations of \emph{unconstrained} 12d gravity, in which $\hat g_{MN}$ is varied as a dynamical 12d metric.
% They do not, and the obstruction can be located precisely.

% A useful diagnostic compares an uplift against a genuine 12d brane sourced by $\hat F_4$.
% For the flux uplifts the only excited sectors of \eqref{12d action} are 12d gravity coupled to $\hat F_4$, with no dilaton.
% The electric solution of Section~\ref{sec:general brane solns} with $D=12$ and $n=4$ (so $d=3$, $\tilde d=7$) has
% \begin{equation}
% \Delta = \frac{2 d\tilde d}{D-2} = \frac{2\cdot 3\cdot 7}{10}=\frac{42}{10},
% \end{equation}
% giving worldvolume and transverse warp exponents $-4\tilde d/[\Delta(D-2)]=-2/3$ and $+4d/[\Delta(D-2)]=+2/7$.
% A genuine 12d electric 2-brane (membrane) would therefore warp its three worldvolume directions \emph{uniformly} by $H^{-2/3}$.
% The F1 uplift instead splits them, as shown in Table~\ref{tab:warp-comparison}.
% \begin{table}[h]
% \centering
% \begin{tabular}{l|cc}
% \hline
% direction & genuine 12d 2-brane & F1 uplift\\
% \hline
% $t,x_1$ & $H^{-2/3}$ & $H^{-3/4}$\\
% $z_1$ & $H^{-2/3}$ & $H^{-1/2}$\\
% transverse $x_i$ & $H^{2/7}$ & $H^{1/4}$\\
% $z_2$ & $H^{2/7}$ & $H^{1/2}$\\
% \hline
% \end{tabular}
% \caption{
% Warp exponents of a genuine 12d electric 2-brane sourced by $\hat F_4$, compared with those of the F1 uplift.
% The genuine solution warps $(t,x_1,z_1)$ uniformly by $H^{-2/3}$, whereas the uplift carries $H^{-3/4}$ on $(t,x_1)$ but $H^{-1/2}$ on $z_1$.
% }
% \label{tab:warp-comparison}
% \end{table}
% Because the worldvolume warps are not uniform, the F1 uplift does not satisfy the 12d Einstein equation $R_{MN}=S_{MN}[\hat F_4]$.

% The mismatch is confined to the two torus directions $z_1,z_2$, which have no 10d counterpart.
% Writing $\det\hat g_{ab}=e^{2\sigma}$ for the torus-volume modulus, the torus-block equations $E_{ab}\equiv R_{ab}-S_{ab}$ decompose into two combinations.
% The volume-preserving combination $\hat g^{z_1z_1}E_{z_1z_1}-\hat g^{z_2z_2}E_{z_2z_2}$ is precisely the 10d axio-dilaton equation of motion, and it vanishes.
% The volume-changing combination is the radion equation
% \begin{equation}
% \hat g^{z_1z_1}E_{z_1z_1}+\hat g^{z_2z_2}E_{z_2z_2}=\frac{\delta S}{\delta\sigma}\propto|\hat F_4|^2\neq 0.
% \end{equation}
% The $10+2$ ansatz freezes the torus volume by imposing $\det\hat g_{ab}=1$, i.e. $\sigma=0$.
% The nonzero radion equation shows that this is not a consistent truncation once 4-form flux is present: if $\sigma$ is held non-dynamical then $\delta S/\delta\sigma=0$ is never imposed and the uplift solves the actual (10d) variational problem, while if $\sigma$ is dynamical the radion is sourced and rolls.

% The radion source $|\hat F_4|^2$ sorts the uplifts into two tiers.
% The no-flux uplifts of Section~\ref{sec:axio-dilaton-12d}, the D7-brane as a 12d KK-monopole and the D(-1)-instanton as a 12d pp-wave, carry no 4-form flux, so the radion source vanishes and they solve even the unconstrained 12d equations: these are genuine 12d solutions.
% The flux uplifts F1, D1, NS5 and D5 source the radion and solve only the embedded 10d equations, so they are ``12d'' solutions in the repackaged sense alone.
% The warp-exponent comparison and the radion decomposition above were verified symbolically.
