...
\end{equation}
Writing $\widetilde{\widehat{F}}_5\equiv\tilde F_5$ for the corresponding torus-leg-free five-form uplift, the reduction in Appendix~\ref{sec:form-field-reduction} gives $\widetilde{\widehat{F}}_7=\widetilde{\widehat{F}}_5\wedge dz_1\wedge dz_2$.
...
Although one often thinks of F1 and NS5, D1 and D5 as EM dual pairs, their $H_3$ and $F_3$ configurations are not related by EM duality in 10d, and of course they have opposite dilaton profiles as well\footnote{The brane solutions are listed in Appendix~\ref{sec:10d IIB branes}.}.
...
% It is a separate question whether they also solve the equations of \emph{unconstrained} 12d gravity, in which $\widehat g_{MN}$ is varied as a dynamical 12d metric.
% They do not, and the obstruction can be located precisely.

% A useful diagnostic compares an uplift against a genuine 12d brane sourced by $\widehat F_4$.
% For the flux uplifts the only excited sectors of \eqref{12d action} are 12d gravity coupled to $\widehat F_4$, with no dilaton.
...
% \end{tabular}
% \caption{
% Warp exponents of a genuine 12d electric 2-brane sourced by $\widehat F_4$, compared with those of the F1 uplift.
...
% \end{table}
% Because the worldvolume warps are not uniform, the F1 uplift does not satisfy the 12d Einstein equation $R_{MN}=S_{MN}[\widehat F_4]$.

% The mismatch is confined to the two torus directions $z_1,z_2$, which have no 10d counterpart.
% Writing $\det\widehat g_{ab}=e^{2\sigma}$ for the torus-volume modulus, the torus-block equations $E_{ab}\equiv R_{ab}-S_{ab}$ decompose into two combinations.
% The volume-preserving combination $\widehat g^{z_1z_1}E_{z_1z_1}-\widehat g^{z_2z_2}E_{z_2z_2}$ is precisely the 10d axio-dilaton equation of motion, and it vanishes.
% The volume-changing combination is the radion equation
% \begin{equation}
% \widehat g^{z_1z_1}E_{z_1z_1}+\widehat g^{z_2z_2}E_{z_2z_2}=\frac{\delta S}{\delta\sigma}\propto|\widehat F_4|^2\neq 0.
% \end{equation}
% The $10+2$ ansatz freezes the torus volume by imposing $\det\widehat g_{ab}=1$, i.e. $\sigma=0$.
...
% The radion source $|\widehat F_4|^2$ sorts the uplifts into two tiers.
