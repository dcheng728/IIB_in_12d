...
\end{equation}
This equation arises directly from the dimensional reduction of the 11d covariantly constant spinor condition $\nabla^{(11)}_M\epsilon = 0$; we provide an explicit derivation in Appendix \ref{sec:IIA killing spinor reduction} (see also \cite{Becker:2006dvp}).\footnote{
We contract gamma matrices with explicit indices, so $F_{np}\Gamma^{np}$ denotes the full sum over $n,p$ (with no $1/2!$). In the slash convention $\slashed{F}_2\equiv\frac{1}{2!}F_{np}\Gamma^{np}$ of \cite{Becker:2006dvp}, the same coupling reads $-\frac{1}{8}e^\phi\slashed{F}_2\,\Gamma_m\Gamma^{11}$; the two are identical, the apparent factor of two being the $1/2!$ absorbed into the slash.}
